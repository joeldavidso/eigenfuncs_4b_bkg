...
def poly_eigfs(means, cov):
    ...
    variation = V @ np.diag(np.sqrt(np.maximum(e,0)))
    # print(f"Max Eigenvalue: {np.max(e):.4e}")
    # print(f"Min Eigenvalue: {np.min(e):.4e}")
    # print(f"Nominal Params Mean: {np.mean(np.abs(means)):.4e}")
...
def reweight(funcs, m1m2s):
    ...
    nom_funcs, var_funcs, scale = funcs
    return scale * nom_funcs(m1m2s), [scale * var_func(m1m2s) for var_func in var_funcs]


def getrunandscale(year):
    runmap = {
        "16": "run2",
        "17": "run2",
        "18": "run2",
    ...
    scalemap = {"16": 0.952, "17": 1.166, "18": 0.946, "run2": 1, "22": 1.005, "23": 0.994, "run3": 1}

    return runmap[year], scalemap[year]


def get3b2b(year, order, path):
    run, scale = getrunandscale(year)

    filepath = path + "3b1j_2b2j__Order-"+str(order)+"/"+run

    means, cov = Read(filepath + "/means", filepath + "/covs")

    nom_func, var_funcs = poly_eigfs(means, cov)

    return nom_func, var_funcs, scale


def get4b2b(year, order, path):
    run, scale = getrunandscale(year)

    filepath = path + "4b_2b2j__Order-"+str(order)+"/"+run
...
def edge_bool(mh1_bins, mh2_bins, region):
...
def Get_MassPlane_Bins(m1_bins, m2_bins, datasets, plot = None):
    ...
    rcr_mask = edge_bool(m1_bins[0], m2_bins[0], "R_CR")
    ...
    for ntag in datasets.keys():
        ...
        for year in datasets[ntag].keys():
            ...
            for region in ["CR", "SR"]:
                ...
                # hists_counts[ntag][year][region][rcr_mask] = 0
                # hists_counts_uncs[ntag][year][region][rcr_mask] = 0
    ...
    if type(plot) == str:
        ...
        for ntag in datasets.keys():
            for year in datasets[ntag].keys():
                ...
                Contour_Plot(rcr_curve,m1_bins[0], m2_bins[0])
...
def Get_Ratio(numerator, hists_counts, m1_bins, m2_bins, plot = None):
    ...
    xhh_mask = edge_bool(m1_bins[0], m2_bins[0], "X_HH")
    rcr_mask = edge_bool(m1_bins[0], m2_bins[0], "R_CR")
    gridpoints = Gridpoints(m1_bins,m2_bins)
    ...
    for year in hists_counts[numerator].keys():
        ...
        for region in hists_counts[numerator][year].keys():
            ...
            hists_ratio[year][region][rcr_mask.flatten()] = np.nan
            hists_ratio_uncs[year][region][rcr_mask.flatten()] = np.nan
    ...
    if type(plot) == str:
        ...
        for ntag in hists_counts.keys():
            for region in ["CR", "SR"]:
                ...
                for year in datasets[ntag].keys():          
                    temp_numerator = np.array([np.sum(hists_counts[numerator][year][region][mh1,:]) for mh1 in range(len(m1_bins[1]))])
                    temp_denominator = np.array([np.sum(hists_counts["2b2j"][year][region][mh1,:]) for mh1 in range(len(m1_bins[1]))])
                ...
                histplot_m1.Plot(plotdir+"m1_"+region)
...
def Get_Bin_Weights(numerator, m1_bins, m2_bins, poly_order, poly_filepath, year_files, plot = False):
    ...
    for year in year_files.keys():
        ...
        for order in poly_orders:
            ...
            binned_weights_uncs[year][str(order)] = 4 * binned_weights_uncs[year][str(order)]
...
def Get_Polynomial_Prediction(numerator, hists_counts, polynomial_weights, polynomial_weights_uncs, m1_bins, m2_bins, plot = None):

    print("Getting "+numerator+" predictions")

    gridpoints = Gridpoints(m1_bins, m2_bins)

    region_masks = {"SR":X_HH(gridpoints[:,0], gridpoints[:,1]) < 1.6,
                    "CR": np.logical_and(X_HH(gridpoints[:,0], gridpoints[:,1]) > 1.6, R_CR(gridpoints[:,0], gridpoints[:,1]) < 45)}
...
def Get_Pulls(numerator, observed, observed_uncs, predicted, predicted_uncs, m1_bins, m2_bins, ratio_count, plot = False):
    ...
    region_masks = {"SR": X_HH(gridpoints[:,0], gridpoints[:,1]) < 1.6,
                    "CR": np.logical_and(X_HH(gridpoints[:,0], gridpoints[:,1]) > 1.6, R_CR(gridpoints[:,0], gridpoints[:,1]) < 45)}
    xhh_mask = edge_bool(m1_bins[0], m2_bins[0], "X_HH").flatten()

    obs = None
    if ratio_count == "ratios":
        obs = observed
        obs_uncs = observed_uncs
    elif ratio_count == "counts":
        obs = observed[numerator]
        obs_uncs = observed_uncs[numerator]

    pulls = {}
    for year in obs.keys():
        pulls[year] = {}
        for order in predicted[year].keys():
            ...
            for region in ["SR", "CR"]:
                pred = None
                if ratio_count == "ratios":
                    pred = predicted[year][order]["nom"]
                    pred_uncs = predicted_uncs[year][order]
                elif ratio_count == "counts":
                    pred = predicted[year][order][region]
                    pred_uncs = predicted_uncs[year][order][region]

                num = obs[year][region].flatten() - pred.flatten()
                den = np.sqrt(obs_uncs[year][region].flatten()**2 + pred_uncs**2)
    ...
    for year in pulls.keys():
        ...
        for order in pulls[year].keys():
            ...
            for region in pulls[year][order].keys():
                chi[year][order][region] = np.sum(np.square(pulls[year][order][region].flatten()[np.logical_and(region_masks[region], np.logical_not(xhh_mask))]))
                n_dof[year][order][region] = np.sum(np.logical_and(region_masks[region], np.logical_not(xhh_mask))) + (int(order)+1)*(int(order)+2)/2 -1
                if not np.isfinite(chi[year][order][region]):
                    chi[year][order][region] = -99999

    if type(plot) == str:
                
        # 2D plot
        Check_Location(plot)
        plotdir = plot + "Pulls_2D_"+ratio_count+"/"
        Check_Location(plotdir)
        plotdir = plotdir + "_"+numerator+"_2b2j/"
        Check_Location(plotdir)
        ...
        for year in pulls.keys():
            for order in pulls[year].keys():
                for region in pulls[year][order].keys():
                    ...
                    temp_pulls = pulls[year][order][region].flatten()[np.logical_and(region_masks[region], np.logical_not(xhh_mask))]

                
                    # 2D plot
                    temp_pull_arr = pulls[year][order][region].flatten()
                    temp_pull_arr[np.logical_not(region_masks[region])] = np.nan 
                    histplot2d = skplt.Hist2D(m1_bins, m2_bins, margins = False, cbar = True,
                                              xlabel = r"$m_{H1} [GeV]$", ylabel = r"$m_{H2} [GeV]$")
        ...
        # 1D plot
        Check_Location(plot)
        plotdir = plot + "Pulls_1D_"+ratio_count+"/"
        Check_Location(plotdir)
        plotdir = plotdir + "_"+numerator+"_2b2j/"
        Check_Location(plotdir)
        ...
        for year in pulls.keys():
            for order in pulls[year].keys():
                for region in pulls[year][order].keys():
                    ...
                    mean, mean_SE = skplt.get_mean(temp_pulls)
                    std, std_SE = skplt.get_std(temp_pulls)
                    # 1D plot
                    ...
                    histplot.Add(temp_pulls, label = "Order-"+str(order))
                    ...
                    histplot.Text(r"$\chi^2/n_{df} = $"+str(round(chi[year][order][region]))+"/"+str(n_dof[year][order][region]), xpos = 0.15, ypos = 0.82)
                    histplot.Plot(plotdir_temp + "/" + year+"_"+region)

    return pulls, chi

def Get_Chi2s(numerator, observed, observed_uncs, predicted, predicted_uncs, m1_bins, m2_bins, N_samples, ratio_count, plot = False):
    ...
    for sample in tqdm(range(N_samples), desc = "sampling", leave = False):
        ...
        for year in observed[numerator].keys():
            ...
            for region in observed[numerator][year].keys():
                    ...
                    elif ratio_count == "ratios":
                        tempnum = np.random.poisson(observed[numerator][year][region])
                        tempden = observed["2b2j"][year][region]
                        temp_observed[numerator][year][region] = np.divide(tempnum, tempden)
                        temp_observed_uncs[numerator][year][region] = temp_observed[numerator][year][region] * np.sqrt(1/tempnum + 1/tempden)
                        

        pulls = None
        chi2s_temp = None
        if ratio_count == "counts":
            pulls, chi2s_temp = Get_Pulls(numerator, temp_observed, temp_observed_uncs, predicted, predicted_uncs, m1_bins, m2_bins, ratio_count)
        elif ratio_count == "ratios":
            pulls, chi2s_temp = Get_Pulls(numerator, temp_observed[numerator], temp_observed_uncs[numerator], predicted, predicted_uncs, m1_bins, m2_bins, ratio_count)
    
        for year in chi2s.keys():
            for order in chi2s[year].keys():
                for region in ["SR", "CR"]:
                    chi2s[year][order][region].append(chi2s_temp[year][order][region])
                         
    if type(plot) == str:

        plotdir = plot + "Chi2s_"+ratio_count+"/"
        Check_Location(plotdir)
        ...
        for year in chi2s.keys():
            for order in chi2s[year].keys():
                ...
                for region in chi2s[year][order].keys():
                    ...
                    ndof = np.sum(region_masks[region]) + (int(order)+1)*(int(order)+2)/2 - 1
...
def Single_Bin_GP(GP_pred, m1_bins, m2_bins):
    ...
    for year in GP_pred["prediction"].keys():
        ...
        print("Predicted 4b SR events from GPs : "+str(round(np.sum(pred_arr)))+"+-"+str(round(np.sum(abs(up_arr)))))
        ...
        print("Predicted 4b CR events from GPs : "+str(round(np.sum(pred_arr)))+"+-"+str(round(np.sum(abs(up_arr)))))
    ...
    for year in ratio_hists.keys():
        ...
        for order in polynomial_weights[year].keys():
            lineplot.Add(polynomial_weights[year][order]["nom"][mh_bin*n_m1:mh_bin*n_m1 + n_m1], uncs = polynomial_weights_uncs[year][order][mh_bin*n_m1:mh_bin*n_m1 + n_m1],
                         label = "Order-"+order, plotunc = True, marker_size = 0)
    ...
    for year in ratio_hists.keys():
        ...
        for order in polynomial_weights[year].keys():
            lineplot.Add(polynomial_weights[year][order]["nom"][mh_bin::n_m2], uncs = polynomial_weights_uncs[year][order][mh_bin::n_m2],
                         label = "Order-"+order, plotunc = True, marker_size = 0, linestyle = "--")
...
def Get_Slices_Counts(numerator, mh_bin, m1_bins, m2_bins, counts_hists, polynomial_predictions, polynomial_predictions_uncs, plot, GP_predictions = None):

    # m1 diff plot
    plotdir = plot+"MassPlane_Diff/"
    Check_Location(plotdir)
    plotdir = plotdir+"Counts/"
    Check_Location(plotdir)
    plotdir = plotdir+"m1/"
    Check_Location(plotdir)
    ...
    for year in counts_hists[numerator].keys():
        plotdir_temp = plotdir + year+"/"
        Check_Location(plotdir_temp)
            
        lineplot = skplt.LinePlot(xs = m1_bins[1], xlabel = r"$m_{H1}$", ylabel = "N("+numerator+")", ratio = True, residual = True)

        resampled_hists = {"CR": 0,"SR": 0}

        for region in counts_hists[numerator][year].keys():
            resampled_hists[region] = np.random.poisson(counts_hists[numerator][year][region].flatten()[mh_bin*n_m1:mh_bin*n_m1 + n_m1])
            # line = counts_hists[numerator][year][region].flatten()[mh_bin*n_m1:mh_bin*n_m1 + n_m1]
            line = resampled_hists[region]
            # line[line < 150] = np.nan
            lineuncs = np.sqrt(line)
            lineplot.Add(line, uncs = lineuncs, label = region+" Obs", linewidth = 0, addthis = False,
                         plotunc = True, marker_size = 5, unctype = "bars")

        for order in polynomial_predictions[year].keys():
            line = polynomial_predictions[year][order]["CR"].flatten()[mh_bin*n_m1:mh_bin*n_m1 + n_m1]
            lineuncs = polynomial_predictions_uncs[year][order]["CR"].flatten()[mh_bin*n_m1:mh_bin*n_m1 + n_m1]
            # line[line < 150] = np.nan
            lineplot.Add(line,
                         uncs = lineuncs,
                         label = "Order-"+order+ " CR Pred", plotunc = True, marker_size = 0)

            line = polynomial_predictions[year][order]["SR"].flatten()[mh_bin*n_m1:mh_bin*n_m1 + n_m1]
            lineuncs = polynomial_predictions_uncs[year][order]["SR"].flatten()[mh_bin*n_m1:mh_bin*n_m1 + n_m1]
            # line[line < 150] = np.nan
            lineplot.Add(line,
                         uncs = lineuncs,
                         label = "Order-"+order+ " SR Pred ", plotunc = True, marker_size = 0)


        if numerator == "4b":
            if not GP_predictions:
                raise("NO GP")
            lineplot.Add(GP_predictions["prediction"][year].flatten()[mh_bin*n_m1:mh_bin*n_m1 + n_m1], uncs = GP_predictions["error_up"][year].flatten()[mh_bin*n_m1:mh_bin*n_m1 + n_m1],
                        label = "GP Pred", plotunc = False, marker_size = 7, marker = "D")

        # ref_line = counts_hists[numerator][year]["CR"].flatten()[mh_bin*n_m1:mh_bin*n_m1 + n_m1] + counts_hists[numerator][year]["SR"].flatten()[mh_bin*n_m1:mh_bin*n_m1 + n_m1]
        # ref_line_uncs = np.sqrt(counts_hists[numerator][year]["CR"].flatten()[mh_bin*n_m1:mh_bin*n_m1 + n_m1] + counts_hists[numerator][year]["SR"].flatten()[mh_bin*n_m1:mh_bin*n_m1 + n_m1])

        ref_line = resampled_hists["CR"] + resampled_hists["SR"]
        ref_line_uncs = np.sqrt(ref_line)
    ...
    for year in counts_hists[numerator].keys():
        ...
        for region in counts_hists[numerator][year].keys():
            resampled_hists[region] = np.random.poisson(counts_hists[numerator][year][region].flatten()[mh_bin::n_m2])
            # line = counts_hists[numerator][year][region].flatten()[mh_bin::n_m2]
            line = resampled_hists[region]
            # line[line < 150] = np.nan
            lineuncs = np.sqrt(line)
            lineplot.Add(line, uncs = lineuncs, label = region+" Obs", linewidth = 0, addthis = False,
        ...
        for order in polynomial_predictions[year].keys():
            line = polynomial_predictions[year][order]["CR"].flatten()[mh_bin::n_m2]
            lineuncs = polynomial_predictions_uncs[year][order]["CR"].flatten()[mh_bin::n_m2]
            # line[line < 150] = np.nan
            lineplot.Add(line,
                         uncs = lineuncs,
                         label = "Order-"+order+ " CR Pred", plotunc = True, marker_size = 0)

            line = polynomial_predictions[year][order]["SR"].flatten()[mh_bin::n_m2]
            lineuncs = polynomial_predictions_uncs[year][order]["SR"].flatten()[mh_bin::n_m2]
            # line[line < 150] = np.nan
            lineplot.Add(line,
                         uncs = lineuncs,
                         label = "Order-"+order+ " SR Pred ", plotunc = True, marker_size = 0)
        ...
        if numerator == "4b":
            if not GP_predictions:
                raise("NO GP")
            lineplot.Add(GP_predictions["prediction"][year].flatten()[mh_bin::n_m2], uncs = GP_predictions["error_up"][year].flatten()[mh_bin::n_m2],
                        label = "GP Pred", plotunc = False, marker_size = 7, marker = "D")

        # ref_line = counts_hists[numerator][year]["CR"].flatten()[mh_bin::n_m2] + counts_hists[numerator][year]["SR"].flatten()[mh_bin::n_m2]
        # ref_line_uncs = np.sqrt(ref_line)
...
if __name__ == "__main__":
    ...
    polyfit_filepath = "../background_estimate/background_estimate/polyfits_info/"
    ...
    m1_bins, m2_bins = skplt.get_bins(80,180,60),skplt.get_bins(70,170,60)
    ...
    # print(np.sum(hists_counts["3b1j"]["run2"]["CR"]))
    # print(np.sum(hists_counts["4b"]["run2"]["CR"]))
    # print(np.sum(hists_counts["3b1j"]["run2"]["CR"])/np.sum(hists_counts["4b"]["run2"]["CR"]))
    ...
    predictions_3b2b, predictions_3b2b_uncs = Get_Polynomial_Prediction("3b1j", hists_counts, bin_weights_3b2b, bin_weights_3b2b_uncs, m1_bins, m2_bins, plot = plot_dir)
    ...
    # Plot Pulls 
    pulls_counts_3b2b, chis = Get_Pulls("3b1j", hists_counts, hists_counts_uncs, predictions_3b2b, predictions_3b2b_uncs, m1_bins, m2_bins, "counts", plot = plot_dir)
    ...
    pulls_ratios_3b2b, chis = Get_Pulls("3b1j", ratios_3b2b, ratios_3b2b_uncs, bin_weights_3b2b, bin_weights_3b2b_uncs, m1_bins, m2_bins, "ratios", plot = plot_dir)

    # Get Chi2s
    chi2s_counts_3b2b = Get_Chi2s("3b1j", hists_counts, hists_counts_uncs, predictions_3b2b, predictions_3b2b_uncs, m1_bins, m2_bins, 1000, "counts", plot = plot_dir)

    chi2s_ratios_3b2b = Get_Chi2s("3b1j", hists_counts, hists_counts_uncs, bin_weights_3b2b, bin_weights_3b2b_uncs, m1_bins, m2_bins, 1000, "ratios", plot = plot_dir)
    ...
    for mh_value in range(m1_bins[1].shape[0]):
        ...
        Get_Slices_Counts("3b1j", mh_value, m1_bins, m2_bins, hists_counts, predictions_3b2b, predictions_3b2b_uncs, plot_dir)

        # Get_Slices_Ratio("4b", mh_value, m1_bins, m2_bins, ratios_4b2b, ratios_4b2b_uncs, bin_weights_4b2b, bin_weights_4b2b_uncs, plot_dir)
        # Get_Slices_Counts("4b", mh_value, m1_bins, m2_bins, hists_counts, predictions_4b2b, predictions_4b2b_uncs, plot_dir, GP_4b_prediction)
